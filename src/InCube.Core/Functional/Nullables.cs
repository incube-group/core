...
namespace InCube.Core.Functional
{
    ...
    public static class Nullables
    {
        ...
        public static T GetValueOrDefault<T>(this T? nullable, T @default)
            where T : class =>
            nullable ?? @default;
        ...
        public static T GetValueOr<T>(this in T? self, Func<T> @default)
            where T : struct =>
            self ?? @default();
        ...
        /// <returns>Nullable value type.</returns>
        public static T? OrElse<T>(this in T? self, Func<T?> @default)
            where T : struct =>
            self ?? @default();
        ...
        public static T? OrElse<T>(this in T? self, T? @default)
            where T : struct =>
            self ?? @default;
        ...
        public static TOut Match<TOut, TIn>(this in TIn? self, Func<TOut> none, Func<TIn, TOut> some)
            where TIn : struct =>
        ...
        public static bool Contains<T>(this in T? self, T elem)
            where T : struct =>
        ...
        /// <returns>True if nullable is not null and it's equal to the input element, false otherwise.</returns>
        public static bool Contains<T>(this in T? self, T elem, IEqualityComparer<T> comparer)
            where T : struct =>
            self.Select(x => comparer.Equals(x, elem)) ?? false;

        /// <summary>
        /// Filters a nullable : if it is null, returns null, if it is not null, checks whether is meets the predicate, if it
        /// doesn't returns null, otherwise returns the same nullable.
        /// </summary>
        /// <typeparam name="T">The type of the nullable value.</typeparam>
        /// <param name="self">The input nullable.</param>
        /// <param name="p">The predicate to check.</param>
        /// <returns>A nullable value type.</returns>
        public static T? Where<T>(this in T? self, Func<T, bool> p)
            where T : struct =>
            self.Any(p) ? self : default;
        ...
        public static bool Any<T>(this in T? self)
            where T : struct =>
            self.HasValue;
        ...
        public static bool Any<T>(this in T? self, Func<T, bool> p)
            where T : struct =>
            self?.Apply(p) ?? false;
        ...
        public static bool All<T>(this in T? self, Func<T, bool> p)
            where T : struct =>
            self?.Apply(p) ?? true;
        ...
        public static void ForEach<T>(this in T? self, Action<T> action)
            where T : struct =>
            self?.Apply(action);
        ...
        public static void ForEach<T>(this in T? self, Action none, Action<T> some)
            where T : struct
        {
            self.ForEach(some);
            if (!self.HasValue)
        ...
        /// <summary>
        /// Applies the <paramref name="map"/> to the value, if it exists, otherwise returns null.
        /// </summary>
        /// <param name="input">The input nullable.</param>
        /// <param name="map">The function to apply to the eventual value.</param>
        /// <typeparam name="TIn">The type of the input.</typeparam>
        /// <typeparam name="TOut">The type of the output.</typeparam>
        /// <returns>A nullable <typeparamref name="TOut"/>.</returns>
        public static TOut? ApplyNullable<TIn, TOut>(this TIn? input, Func<TIn, TOut?> map)
            where TIn : struct
            where TOut : class =>
        ...
        public static TIn? FirstNullable<TIn>(this IEnumerable<TIn> inputs, Func<TIn, bool> predicate)
            where TIn : struct
        {
